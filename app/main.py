...
def results(filename): 
    ...
    (class_ids, labels, boxes, confidences) = yolo_forward(net, LABELS, image, confidence_level=0.25)
    ...
    if len(class_ids) > 0:
        ...
        print(confidences)
        truncated_filename = new_filename.split('.')[0]
        text = '''
            <html>
            <body>
            <h1>''' + labels + '''<p>'''+ confidences +'''<p/></h1>
            </body>
            </html>
            '''

        file = open('images/' + truncated_filename + ".html","w")
        file.write(text)
        file.close()
        ...
        return render_template('results.html', labels='No Buildings', old_filename=filename, filename=filename) 
